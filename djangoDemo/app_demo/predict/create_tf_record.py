...
def read_image(filename, resize_height, resize_width,normalization=False):
    ...
    :return: 返回的图片数据
    '''
    # bgr_image = cv2.imread(filename)  #该函数不支持中文路径
...
def read_image_base64(img_base64, resize_height, resize_width,normalization=False):
    '''
    读取图片数据,默认返回的是uint8,[0,255]
    :param filename:
    :param resize_height:
    ...
    image_data = np.fromstring(img_bytes, np.uint8)
    bgr_image = cv2.imdecode(image_data, cv2.IMREAD_COLOR)
    if len(bgr_image.shape)==2:#若是灰度图则转为三通道
        raise("Warning:gray image")
        bgr_image = cv2.cvtColor(bgr_image, cv2.COLOR_GRAY2BGR)
...
def create_records(file, output_record_dir, resize_height, resize_width,shuffle,log=5):
    ...
    :param file:输入保存图片信息的txt文件(file图片的路径)
