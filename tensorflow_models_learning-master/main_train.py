...
def net_evaluation(sess,loss,accuracy,val_images_batch,val_labels_batch,val_nums):
    # val_max_steps = min(int(val_nums / batch_size), 50)
    val_max_steps = 50
    val_losses = []
    val_accs = []
    ...
    with tf.Session() as sess:
        ...
        for i in range(restore_steps, max_steps + 1):  
            ...
            # if i % val_log_step == 0:
                ...
                feed_data_to_file(i, train_loss, train_acc, mean_loss, mean_acc)
...
def train(train_record_file,
    ...
    train_images_batch, train_labels_batch = get_batch_images(train_images, train_labels,batch_size=batch_size, labels_nums=labels_nums,one_hot=True, shuffle=True)
    # val数据,验证数据可以不需要打乱数据
    val_images, val_labels = read_records(val_record_file, resize_height, resize_width, type='normalization')
    val_images_batch, val_labels_batch = get_batch_images(val_images, val_labels,batch_size=batch_size, labels_nums=labels_nums,one_hot=True, shuffle=False)
    ...
    with tf.control_dependencies(update_ops):
        ...
        train_op = slim.learning.create_train_op(total_loss=loss, optimizer=optimizer,global_step=global_steps)
